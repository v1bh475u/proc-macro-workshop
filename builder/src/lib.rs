...
mod builder {
    ...
    pub(crate) fn generate_builder(input: &DeriveInput) -> Result<TokenStream> {
        ...
        let fields: Vec<BuilderField> = fields
            .iter()
            .map(|f| -> Result<BuilderField> {
                let name = f.ident.as_ref().unwrap();
                let kind = classify_type(&f.ty);

                let each = extract_builder_each(&f.attrs)?;

                Ok(BuilderField { name, kind, each })
            })
            .collect::<Result<_>>()?;

        for field in &fields {
            if field.each.is_some() {
                match field.kind {
                    TypeKind::Vec(_) => {}
                    _ => {
                        return Err(syn::Error::new_spanned(
                            field.name,
                            "`each` is only allowed on fields of type Vec<T>",
                        ));
                    }
                }
            }
